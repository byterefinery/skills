...
```bash
rqlited -cdc-config stdout /data/node1
```
...
|---|---|---|---|
| `endpoint` | string | *(required)* | HTTP URL for CDC events, or `stdout` |
...
| `transmit_max_retries` | int | unlimited | Max retries; drop event after this many failures |
