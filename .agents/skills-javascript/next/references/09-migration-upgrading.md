...
13. **ESLint 9** is supported (flat config); `next lint` is deprecated in 15 (removed in 16) — plan to run ESLint directly.
...
15.x releases are additive; watch the release notes for: new `next lint` deprecation warning (15.5), PPR/`ppr` flag behavior, `use cache` directive (experimental), typed routes. Pin a minor you've tested and upgrade deliberately.
