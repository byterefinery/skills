...
- Memoization: identical `GET` fetches in the same render pass deduplicate automatically (React `cache`), so the same data can be fetched independently in a layout, page, and child components without extra cost.
...
`process.env.X` in a Server Component is **inlined at build time** into the serialized RSC payload if that value reaches a client boundary (e.g. as a prop). Never pass secrets from the server tree to the client. Client-only variables must start with `NEXT_PUBLIC_` (see 10-cli-config for the env rules).
