...
- **Remote images** require config (15 uses `remotePatterns`; the old `domains` option is gone):
...
- `fill` + `sizes` for responsive cover images; `priority` for above-the-fold (disables lazy loading); `placeholder="blur"` + `blurDataURL` for blur-up; `loading="lazy"` (default) / `"eager"` / `"lazy"`.
...
- **Sass/Less** — supported out of the box (`.scss`/`.less`).
