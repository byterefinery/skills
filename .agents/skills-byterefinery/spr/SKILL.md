...
- **Single-language documents** — keep all primings in the source language. Never translate or rewrite into another language. The compressor distills and compresses, but never changes language.
...
**Always preserve the language of the source content.** Never translate, normalize, or rewrite primings into a different language. For single-language documents, keep all primings in that language. For multi-lingual documents, keep each section's primings in whichever language that section was originally written — a paragraph in French stays in French, a paragraph in Japanese stays in Japanese, a code comment in English stays in English. When the document switches languages, mark the switch explicitly with a language tag (`[lang:de]`, `[lang:ja]`, `[lang:en]`, etc.) so the decompressor reproduces the same pattern. Language choice carries semantic weight: legal terms, technical jargon, cultural references, and nuance often lose precision when translated. The compressor's job is to distill, not to translate.
...
7. **Preserve source language** — keep every priming in the language of its source content. For single-language documents, all primings stay in that language. For multi-lingual documents, each section stays in its original language — note switches with `[lang:xx]` tags. Never translate, normalize, or rewrite primings into a different language. Language choice carries semantic weight.
...
- **Source language is always preserved** — language choice is not incidental. Legal terms, technical jargon, and cultural references often lose precision when translated. Keep primings in the original language of the source content, whether single-language or multi-lingual. Note language switches explicitly with `[lang:xx]` tags.
...
- **SPR output stays in the source language(s)** — the compressed file is always written in the language(s) of the source. For single-language documents, all primings stay in that language. For multi-lingual documents, each section stays in its original language. Language tags (`[lang:xx]`) mark switches for the decompressor. Never translate, normalize, or rewrite primings into a different language — the compressor distills and compresses, but never changes language.
